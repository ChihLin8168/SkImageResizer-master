    ...
    public class SKImageProcess
    {
        ...
        public async Task ResizeImagesAsync(string sourcePath, string destPath, double scale)
        {
            ...
            foreach (var filePath in allFiles)
            {
                ...
                tasks1.Add(Task.Run(() => outputAsync(destPath, scale, filePath)));
               
            }
            await Task.WhenAll(tasks1);
        }

        private static async Task outputAsync(string destPath, double scale, string filePath)
        {
            List<Task> tasks1 = new List<Task>();
            ...
            var destinationWidth = (int)(sourceWidth * scale);
            var destinationHeight = (int)(sourceHeight * scale);
            scaledBitmap = bitmap.Resize(
                                new SKImageInfo(destinationWidth, destinationHeight),
                                SKFilterQuality.High);
            scaledImage = SKImage.FromBitmap(scaledBitmap);
            data = scaledImage.Encode(SKEncodedImageFormat.Jpeg, 100);
